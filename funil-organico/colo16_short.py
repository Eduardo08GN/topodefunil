...
# -*- coding: utf-8 -*-
"""
colo16_short.py — randomizador + gerador + linter do AGENTE **COLO 16**.

⭐⭐ O QUE ELE E': o COLO em **DOIS takes de 8s = 16 segundos**. Setimo motor da
familia 16s, sob a mesma clausula do operador: ajuste so' no eixo temporal.

⛔ Ele **nao substitui** o `colo_short.py`. Ledger proprio, copia literal.

⭐ O ANGULO: a isca no colo. Prop falico em pe' no punho dela, acima do colo e
entre os joelhos, com substancia absurda despejada por cima — e ela desmente a
propria promessa na mesma respiracao. Fonte: Sofia Maren, reel 1580259273673843.

O ARCO — 2 cenas de 8s, destino AdBatch Vertical 2:

    cena 1  A ISCA NO COLO  sentada na sala, o prop no punho + o desmentido
    cena 2  A PROVA         a cozinha da mesma casa: a bancada da receita, o
                            copo pronto e a GELATINA na mao · o gelatin trick
                            + o CTA

⭐ COMO AS 3 VIRARAM 2. As cenas 2 e 3 do COLO ja' acontecem na mesma cozinha
(a cena 3 abre com `same place, same background`), entao a bancada e o payoff
cabem no mesmo frame sem inventar cenario.

⛔⛔ O QUE MORREU: o DESPEJO em cena. No motor de 24s a mao direita dela segura
a colher acima do copo e o po' esta' caindo; a esquerda apoiada. Na fusao as
maos vao para a GELATINA, que e' o objeto da keyword deste angulo — e ele tem
de estar NA MAO no frame em que a boca diz `gelatin,`.
⭐ A bancada continua em quadro com o layout sorteado: o que se perde e' o
gesto, nao o repertorio.

⚠️ MEDIDO no `colo_short.py` de 24s, antes de portar: a cena 3 de la' tem 1.260
combinacoes e so' 182 cabem (14%) — SETE das quatorze ROTINAS sao
inalcancaveis, e a cena entrega 139 falas distintas em 300 videos. Aqui a mesma
cena da' 42% com ZERO entrada inalcancavel.

⛔ A VIRGULA DEPOIS DE `gelatin` E' INTOCAVEL, e o PRONOME tem de ter DONO —
...
# ⛔ LEDGER PROPRIO: 24s e 16s sao lotes diferentes e cada um tem de
# varrer o repertorio inteiro sem gastar o frescor do outro.
LEDGER = os.path.join(AQUI, ".colo-16-ledger.json")

TITULO = "AGENTE COLO 16"
SLUG = "colo-16"
...
# deu pronome sem dono e a MESMA FORMA em 62% das entradas. Sao tres eixos, e
# dois deles sao os DO PROPRIO MOTOR — `ISCAS_ENTREGA` (10) e `GATES` (9)
...
# ⛔ O piso da cena 2 e' ARITMETICA: menor PROVA + nucleo do CTA + menor
# ISCA + menor GATE. Piso abaixo disso e' alarme que nunca toca.
PISO_FALA = {1: 20, 2: 20}
...
EIXOS_UI = [
    ...
    # ⛔⛔ O EIXO `receita` SAIU DO PAINEL — 2026-08-08, e isso e'
    # honestidade, nao economia. Ele vivia SO' na fala da cena 2 do
    # motor de 24s (`spec["receita"]` + ancora), e no 16 essa fala virou
    # prova + CTA + gate. O `layout` da bancada e' string FIXA, entao a
    # receita nao chega a quadro nenhum.
    # ⚠️ MEDI a alternativa antes de tirar: com a receita no lugar da
    # prova, so' 11% das combinacoes cabem, DEZ entradas ficam
    # inalcancaveis, e nenhuma das dez RECEITAS carrega o literal
    # `gelatin trick` nem o orgao — as duas coisas que a cena 2 tem de
    # entregar. O eixo nao coube; nao foi descartado por conveniencia.
    # ⛔ Eixo no painel que nao muda o video e' PIOR que eixo ausente: o
    # operador troca, le' o prompt, ve' que nada mudou, e para de
    # confiar no painel inteiro. Por isso ele sai — e o
    # `lint_painel_honesto` e' quem cobra.
]
...
def montar(spec):
    ...
    # --- CENA 2 — A BANCADA, O COPO PRONTO E A GELATINA NA MAO -------------
    # ⭐⭐ E' A FUSAO DAS CENAS 2 E 3, e ela nao inventa cenario: a cena 3 ja'
    # abria com `same place, same background`.
    # ⛔⛔ O DESPEJO NAO ENTRA. No motor de 24s a mao direita dela segura a
    # colher acima do copo e o po' esta' caindo; a esquerda apoiada na bancada.
    # Aqui as maos vao para a GELATINA — que e' o objeto da keyword deste
    # angulo e tem de estar NA MAO no frame em que a boca diz `gelatin,`.
    # ⭐ A bancada FICA, com o layout sorteado: o que se perde e' o gesto, nao
    # o repertorio.
    b["IMAGE 02/02"] = (
        "Closer medium shot in %(banc)s, same light, filmed straight on and "
        "framed from the waist up. %(Ancora)s, standing centred in the frame "
        "behind %(sup_a)s. On the %(sup)s along the bottom edge of the frame, "
        "at frame-left, stands a tall glass filled with the finished drink and "
        "no longer clear, and beside it %(layout)s. %(keyword)s She looks "
        "directly into the lens, calm and confident, one corner of her mouth "
        "raised in a half-smile, her mouth open mid-word as she speaks. She is "
...
def lint(spec, blocos):
    ...
    # ⚠️ `finditer`, nao `search`: `gelatin` aparece DUAS vezes na fala, e
    # olhar so' a primeira dava verde para `Comment gelatin now,`.
    _f2 = falas[1] or ""
    ...
    # a lente rodava em dobro sobre a mesma coisa. Uma cena, uma checagem.
    ...
    if "only his legs and his hands" not in i1:
        ach.append(("ERRO", "CO13: IMAGE 01/02 sem o recorte do homem — sem ele "
    ...
    if "his head and upper body out of shot" not in i1:
        ach.append(("ERRO", "CO13: IMAGE 01/02 nao mantem a cabeca e o tronco "
    ...
    for tok in ("lap", "thigh", "thighs", "knees apart", "between his knees",
        ...
        alvo = "%s %s" % (i1.lower(), blocos["TAKE 01/02"].lower())
    ...
    if "only person" in i1 or "only person" in blocos["TAKE 01/02"]:
    ...
    for nome in ("IMAGE 02/02", "IMAGE 02/02", "TAKE 02/02", "TAKE 02/02"):
    ...
    # --- CO5: a gelatina em cubos so' na cena do PAYOFF --------------------
    # ⛔⛔ REAPONTADA — a fusao INVERTEU a regra. Ela varria as cenas 1 e 2
    # PROIBINDO a gelatina, porque no motor de 24s o payoff era a cena 3.
    # Aqui a cena 2 E' o payoff: a gelatina esta' na mao dela e e' o objeto
    # da keyword. A proibicao reprovava 600 de 600.
    # ⚠️ Renomear `/03` para `/02` em bloco troca o NOME do bloco e nao sabe
    # que a FUNCAO dele mudou. Quarta vez nesta serie de portes.
    if "gelatin" in i1.lower():
        ach.append(("ERRO", "CO5: gelatina na cena 1 — entrega o payoff "
                            "antes da promessa"))
    if "gelatin cubes" not in i2:
        ach.append(("ERRO", "CO5: a cena 2 tem de mostrar a gelatina em cubos "
                            "na mao — e' o objeto da keyword, e a boca diz "
                            "`gelatin,` com ela no frame"))
...
def autoteste(n=600):
    ...
    # ⚠️ MEDIDO no `colo_short.py` de 24s: a cena 3 de la' tem 1.260
    # combinacoes e so' 182 cabem (14%) — SETE das quatorze ROTINAS sao
    ...
    # [CO7] ancora removida
    b5 = dict(b)
    # ⛔ A SONDA ESTAVA CEGA: ela trocava `the same N-year-old` em
    # MINUSCULA, e o bloco traz `The same` com maiuscula — o replace nao
    # casava nada e a sabotagem nunca chegava na regra. Sonda que nao
    # altera o alvo passa sempre, e passar sempre e' o mesmo que nao
    # existir. Achada pelo proprio autoteste, que a reportou como CEGA.
    b5["IMAGE 02/02"] = re.sub(
        r"(?i)the same %d-year-old" % s["ref"]["idade"], "a",
        b5["IMAGE 02/02"])
