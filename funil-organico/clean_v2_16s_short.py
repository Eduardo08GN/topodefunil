...
⭐⭐⭐ E ESTE ARQUIVO E' O V2 EM **DOIS TAKES** (16 SEGUNDOS) — 2026-08-09.
Ordem do operador: *"comece a fazer uma versao de 16seg do agente clean V2,
igual voce fez com a V1"*. Mesma regra de la': **o ajuste e' so' no eixo
temporal, sem perda de entropia**. Nenhum pool encolheu.

⛔ O `clean_short_v2.py` ORIGINAL NAO FOI TOCADO. Este e' agente NOVO, com
ledger proprio e teto de 30 posts proprio.

⭐ TODA A MAQUINARIA DO V2 VEIO INTEIRA — e' o que separa este arquivo de uma
copia do `clean_v1_16s_short.py`:
    · ETNIA LIVRE (pool `ETNIAS`, 14) desamarrada da pagina
    · MUNDOS (20) + `FAMILIAS_MUNDO` + trajes derivados do mundo
    · `EIXOS_TRAVAVEIS` — o cadeado por eixo no painel
    · `PELE_TRAVAVEL` / `PELE_VETO` / `SEM_PALAVRA_RACA`
    · `refs_do_sexo` com `.recebe_spec`, o conserto do botao `quem fala`

⭐ E TODA A COPY DE 16 SEGUNDOS VEIO DO V1 16SEG, onde ela foi aprovada linha
a linha pelo operador em 2026-08-08. ⚠️ Isso e' seguro porque foi MEDIDO: os
pools de copy do `clean_short.py` e do `clean_short_v2.py` sao identicos
objeto a objeto (HOOKS, VIRADAS, CTAS, GATES, NUCLEO, TRUQUE, FAMILIAS) — o
V2 nunca divergiu na fala, so' na cena e no painel.

O QUE MUDOU EM RELACAO AO V2 DE 3 CENAS
  · 3 cenas de 8s  ->  2 cenas de 8s (teto de 20 palavras por take, ~7s de
    narracao com 1s de margem, que foi a margem que ele pediu)
  · a cena 2 do V2 (ITEM_A + ITEM_B + VIRADA) some como CENA; a virada migra
    para o take 2 e o item entra nele quando cabe. Os dois pools continuam
    sorteados e continuam montando a BANCADA — caiu a fala, nao a entropia
    visual
  · HOOKS 14 -> 30 · CTAS reescritos (todos nomeiam o formato VIDEO e
    PROMETEM a entrega) · GATES comprimidos de 8-11 para 2-7 palavras
  · 16S6 (2026-08-09): ancora de TRILHA reforcada e ancora de RITMO
...
Uso:
    python funil-organico/clean_v2_16s_short.py --pagina chuck --n 1
"""
...
LEDGER = os.path.join(AQUI, ".clean-v2-16s-ledger.json")

TITULO = "AGENTE CLEAN V2 16SEG"
SLUG = "clean-v2-16s"
SUBTITULO = ("a fileira apontada, em 2 cenas · etnia livre + trava por eixo · "
...
# ⭐⭐ 16S6 (2026-08-09) — DUAS ANCORAS NASCIDAS DE RELATO DE CAMPO, importadas
# do V1 16SEG, onde nasceram no mesmo dia. As duas tem a MESMA causa raiz do
# CL31 logo acima: a regra estava escrita, so' que na posicao mais fraca do
# prompt e na forma que o gerador menos respeita.
#
...
SEM_TRILHA = (
    ...
    "never music")
# ⚠️ `%(S)s` so' no INICIO de frase e `%(s)s` no meio — com o pronome maiusculo
# nas tres posicoes sai *"on purpose: She says it"*, defeito que nao quebra
# nada e so' se ve lendo o bloco pronto.
RITMO = (
...
for _t in ("no song", "no soundtrack", "no score", "no background music",
           "no melody", "no beat"):
    assert _t in SEM_TRILHA, "16S6: ancora de trilha sem o sinonimo %r" % _t
assert "raw sound recorded live" in SEM_TRILHA, (
    "16S6: ancora de trilha sem a ancora POSITIVA de som cru")
...
def sortear(pagina, rng, led, travas=None):
    ...
    # ⭐ MODO BELA / MODO FORTE — ver a nota do clean_short.
    if ref_trav:
        ref = ref_trav
    elif sexo == "homem":
    ...
    # ⛔ 16S7 — o pool de hook depende do MUNDO (ver `_hooks_do_mundo`).
    hook = rng.choice(_hooks_do_mundo(mundo)).format(o=orgaos[0])
    ...
    return {
        ...
        # ⭐ 16S — DUAS falas. A cena 2 do V2 (item A + item B + virada) deixou
        ...
        # BANCADA — a fileira em cena nao perdeu um item sequer, e `item_b`
        # segue mandando no `_derivar_cena` logo acima.
        "falas": [hook, fala2],
    }


def _hooks_do_mundo(mundo):
    """Os HOOKS que nao nomeiam uma superficie ausente daquele mundo.

    ⛔⛔ 16S7 (2026-08-09) — CINCO dos 30 hooks dizem `counter`. Eles nasceram
    no V1 16seg, onde a superficie e' SEMPRE um balcao. O V2 tem 20 mundos e
    **14 deles sao `table`**: em 70% dos sorteios o hook falava de um balcao
    que a IMAGE nao desenha. Contradicao entre fala e imagem e' a mesma familia
    do 16S5 — o gerador resolve mexendo no que estava certo.

    ⚠️ ACHADO PELO `autoteste()` DO PROPRIO V2, invariante [2], em 62 de 600
    sorteios. Nao apareceu na leitura de um lote nem no linter herdado: a lente
    que existia para isso e' de mundo, e ela so' rodou porque o V2 a trouxe.

    ⛔ NAO SE REESCREVE A COPY APROVADA PARA RESOLVER ISSO — e' alcada do
    operador. O que se faz aqui e' nao SORTEAR o hook que nao cabe naquele
    mundo, exatamente como `_viradas_que_cabem` descarta a virada que estoura
    o teto. Nenhuma string mudou; o que mudou foi onde cada uma pode sair.
    ⚠️ Custo medido: nos 14 mundos de mesa o pool de hook vai de 30 para 25.
    Parametrizar a superficie (`this {sup}`) devolveria os 5 — mas isso e'
    mudar copy, e a decisao e' dele.
    """
    sup = mundo["sup"]
    pool = [h for h in HOOKS
            if "{o}" in h and (sup == "counter" or "counter" not in h)]
    # ⛔ SEM REDE SILENCIOSA. Se um dia o filtro zerar o pool, estourar aqui com
    # o motivo escrito e' melhor do que cair no `or HOOKS` e devolver um hook
    # que contradiz a cena — a rede que "conserta" e' a que esconde. Medido:
    # com os 30 hooks de hoje sobram 25 no pior mundo.
    assert pool, ("16S7: nenhum hook cabe no mundo %s (superficie %s)"
                  % (mundo["id"], sup))
    return pool
...
def _montar_take2(item, orgao, rng):
    ...
    ⚠️ A virada abre com `But`, que so' faz sentido depois do item. Sem item,
    o `But` cai e a frase e' recapitalizada — senao o take 2 abriria com um
    "Mas" pendurado no nada.
    """
    def cabe(t):
...
def montar(spec):
    """Os 5 blocos (REF + 2 IMAGE + 2 TAKE). ⚠️ montar() e' o UNICO ponto que
    olha spec['familia'] — sortear() e o banco de copy sao identicos nas duas
    (CL16)."""
    ...
    if fam == "aponta":
        v.update({"fila": _fila(spec["bancada"]),
                  ...
                  # ⭐ 16S5 — o primeiro objeto da cena final e' o ingrediente
                  # CITADO na fala. Sem `citado`, volta a ser o da bancada.
                  "banc0": VISUAL[citado or spec["bancada"][0]]})
        ...
        mov = [
            ...
            # ⛔⛔ 16S4 — ESTA FRASE ERA GENERICA E MENTIA. Falava em `The
            # glass, the bowl of gelatin cubes and the box`, mas a familia
            # `aponta` nao tem copo nem caixa: a IMAGE mostra o item da
            # bancada, a gelatina e o item do truque. Achado LENDO o lote, nao
            # pelo linter — em 3 cenas passava por ser 1 de 3; aqui e' METADE
            # do video. Agora a frase e' DERIVADA dos mesmos tres objetos que
            # a IMAGE final desenha, entao ela nao pode divergir.
            # ⚠️ `%(sup)s`, nunca o literal `counter` — invariante [2] do
            # autoteste: em mundo de mesa, `counter` e' literal esquecido.
            "The three things on the %(sup)s, the bowl of gelatin cubes among "
            "them, stay exactly as they appear in the first frame — nothing "
            "moves, nothing is touched." % v,
        ]
    else:
        ...
        v.update({
             ...
             # ⭐ 16S5 — o terceiro objeto da cena final e' o ingrediente
             # CITADO na fala. Sem `citado`, volta a ser o do despejo.
        ...
        mov = [
            ...
            # (o pote do despejo), e desde que a cena final passou a mostrar o
            ...
            # 400 sorteios do V1 16seg. Contradicao entre TAKE e IMAGE e' pior
            # que omissao — o Veo resolve mexendo no que estava certo.
    ...
    # continuam sendo MONTADOS acima, com o mesmo codigo validado do V2, e a
    ...
    # (o CL27 da cena 1, o CL21 da final, os invariantes de mundo).
    ...
    # exatamente aquele literal, e as duas linhas saem como saiam antes.
    amb = mundo["audio"]
    if fam == "preparo":
        audio = ["%s, %s" % (amb, DESPEJO[spec["despejo"][0]]["som"]), amb]
    else:
        audio = [amb] * 2
    # ⭐ CL29 — a voz entra nas DUAS linhas, num lugar so'. Ancorar so' numa
    # deixaria a regra dependendo de qual cena derrapa.
    ...
    audio = ["%s. %s. %s." % (VOZ, a[:1].upper() + a[1:], SEM_TRILHA)
             for a in audio]
...
def lint(spec, blocos):
    ...
    # ⭐⭐ 16S1 — o CTA nomeia o VIDEO **e** promete a entrega. Citar o video
    # sem prometer o envio foi reprovado pelo operador em 2026-08-08:
    # `the video shows...` descreve, nao entrega.
    ...
    for nome in ("IMAGE 01/02",):
        if "gelatin cubes" in blocos.get(nome, ""):
            ach.append(("ERRO", "CL21: gelatina fora da cena 2 (%s)" % nome))
    ...
    # ⛔ 16S7 — a FALA nao nomeia superficie que nao esta' em cena. A invariante
    # [2] do autoteste ja' varre o bloco inteiro, mas ela so' roda no autoteste;
    # o operador olha e' o painel, e o painel roda o lint.
    if spec["mundo"]["sup"] != "counter" and "counter" in " ".join(falas):
        ach.append(("ERRO", "16S7: a fala diz 'counter' e o mundo %s tem %s — "
                            "hook de balcao em cena de mesa"
                            % (spec["mundo"]["id"], spec["mundo"]["sup"])))
    ...
    for nome in ("TAKE 01/02", "TAKE 02/02"):
        ...
        if not aud.startswith(VOZ):
            ach.append(("ERRO", "CL31: %s — o campo Audio nao ABRE com a voz"
                                % nome))
        # ⛔ LITERAIS, nunca `endswith(SEM_TRILHA)`. A primeira versao no V1
        # 16seg cobrava a saida CONTRA A PROPRIA CONSTANTE: enfraquecer a
        # ancora mudava os dois lados da comparacao e a lente aprovava calada.
...
def resumo_pt(spec):
    ...
    m = spec["mundo"]
    return ("%s %s de %d anos, de %s %s, em %s (%s). Na bancada: %s. %s. Duas "
...
def nova_fala(spec, i, rng):
    ...
    o = spec["orgaos"]
    if i == 0:
        # ⛔ 16S7 aqui TAMBEM. O botao `trocar` nao passa por `sortear()`, e
        # filtrar so' la' deixaria o operador reintroduzir o balcao fantasma
        # com um clique — foi assim que o CL20 escapou pelo `trocar` de item.
        return rng.choice(_hooks_do_mundo(spec["mundo"])).format(o=o[0])
...
def autoteste(n=600):
    ...
    try:
        ...
        # ⛔⛔ E A PARIDADE E' DO QUE CHEGA AO VIDEO, NAO DO DICIONARIO INTEIRO.
        # 2026-08-13: o `rotulo` (texto do painel, em portugues) nasceu aqui
        # para o menu suspenso da familia 16s e NAO existe no `clean_short`, que
        # e' de 24s — propaga-lo so' para calar esta lente seria mexer num motor
        # fora do escopo do pedido. A lente passou a comparar os campos que o
        # gerador le' (`idade`, `cabeca`, `marca`, `corpo`...), que sao os que
        # fazem dois lotes sairem diferentes meses depois.
        # ⚠️ A regra de qual campo ignorar mora AQUI, numa lista nomeada: um
        # `!=` frouxo deixaria passar divergencia de verdade.
        _SO_PAINEL = ("rotulo",)

        def _do_video(pool):
            return [{k: v for k, v in e.items() if k not in _SO_PAINEL}
                    if isinstance(e, dict) else e for e in (pool or [])]

        for _nome in ("CORPOS_H", "CORPOS_M", "REFS_M", "REFS_H"):
            if _do_video(getattr(_v1, _nome, None)) != _do_video(globals()[_nome]):
