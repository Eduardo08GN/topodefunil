...
# -*- coding: utf-8 -*-
"""
dupla16_short.py — randomizador + gerador + linter do AGENTE **DUPLA 16**.

⭐⭐ O QUE ELE E': o DUPLA em **DOIS takes de 8s = 16 segundos**. Segundo motor
da familia 16s, depois do TRIO 16, e nasce sob a mesma clausula do operador:

    *"O ajuste e' apenas no eixo temporal: nao sacrifique, diminua ou faca
    quaisquer regressao. Os recursos atuais de pool, controladores, etc, devem
    continuar preservados."*

⛔ Ele **nao substitui** o `dupla_short.py`. Os dois coexistem, ledger proprio
cada um. Copia literal + cirurgia temporal.
...
o payoff da cena 3 cabem no mesmo frame sem inventar cenario nenhum.

⛔⛔ E AQUI NADA MORRE, ao contrario do TRIO 16. La' a amiga teve de sair do
quadro 2 para o corpo-prova masculino caber; aqui NAO HA' HOMEM — o operador o
removeu deste angulo em 2026-08-05 (*"o take final deve ter as duas mulheres
novinhas lindas com uma delas segurando o geoduck ereto e bem grande"*). Sobram
duas pessoas e a bancada, que e' menos gente do que o TRIO fundido tem.
⚠️ Por isso `REACOES_AMIGA` continua VIVO nas duas cenas.

⛔ A DIFERENCA DE COPY QUE SO' O DUPLA TEM: o `ISCAS_ENTREGA`, 18 entradas que
variam O QUE o CTA promete entregar (`the whole recipe`, `the exact
measurements`, `the missing step`). O TRIO 16 tem payload fixo; aqui ele e'
sorteado, e por isso a cena 2 tem QUATRO eixos em vez de tres.

⚠️ ONDE MORA O `gelatin trick`, medido em 600 sorteios do DUPLA SHORT antes de
decidir: cena 1 = 234/600, cena 2 = **600/600** (nas ANCORAS), cena 3 = 0/600.
Ou seja, o literal e' garantido pela cena 2 — e a cena 2 e' justamente a que
tem a fala reescrita aqui. Por isso TODA entrada de `PROVAS` o carrega, como
sujeito.

⛔ E A VIRGULA DEPOIS DE `gelatin` E' INTOCAVEL, pela mesma razao do TRIO 16: o
operador confirmou em 2026-08-08 que a automacao de DM casa palavra EXATA. O
comando e' `Comment gelatin, and I'll send you <isca>.` e o follow vem em frase
SEPARADA. Lente T16-2.

Uso:
    python funil-organico/dupla16_short.py --pagina joe --n 1
    python funil-organico/dupla16_short.py --autoteste
"""
...
# ⛔ LEDGER PROPRIO. Compartilhar com o DUPLA SHORT faria o frescor de um gastar
# o do outro: 24s e 16s sao lotes DIFERENTES e cada um tem de varrer o
# repertorio inteiro.
LEDGER = os.path.join(AQUI, ".dupla-16-ledger.json")

TITULO = "AGENTE DUPLA 16"
SLUG = "dupla-16"
...
# ⭐ A CAUDA DA BANCADA DO 16 — uma so' para as seis conformacoes, e ela vive
# FORA das entradas de proposito: o campo `aparato16` tem de ser literal PURO,
...
PREPAROS = [
    ...
    {"id": "espremedor",
     "bancada": "a stainless juicer with a tall clear glass under its spout, "
                ...
                "a shallow bowl of vivid purple gelatin cubes sitting untouched at the edge",
     "aparato16": "a stainless juicer with red-stained pulp still in its catcher "
                   "and the plunger resting in the chute",
    ...
    {"id": "coador",
     "bancada": "a fine mesh sieve set over a tall clear glass, deep red pulp "
                ...
                "a shallow bowl of vivid purple gelatin cubes sitting untouched at the edge",
     "aparato16": "a fine mesh sieve resting over a wide jug with deep red pulp "
                   "still in the mesh and a spoon beside it",
...
REFS = [
    {"idade": 24, "corpo": "tall and long-legged with a very small waist",
     "id": "auburn_ondas",
     "rotulo": "24y · auburn ondas longas + sardas",
     "id": "auburn_ondas",
     "rotulo": "24y · auburn ondas longas + sardas",
    ...
    {"idade": 27, "corpo": "slim with an hourglass figure and long legs",
     "id": "cobre_rabo",
     "rotulo": "27y · rabo cobre + pinta no labio",
     "id": "cobre_rabo",
     "rotulo": "27y · rabo cobre + pinta no labio",
    ...
    {"idade": 23, "corpo": "willowy and fine-boned with a flat stomach",
     "id": "ginger_camadas",
     "rotulo": "23y · ginger repicado + sardas fortes",
     "id": "ginger_camadas",
     "rotulo": "23y · ginger repicado + sardas fortes",
    ...
    {"idade": 29, "corpo": "curvy with a narrow waist and full shoulders",
     "id": "vermelho_escuro",
     "rotulo": "29y · ruiva escura + argola na narina",
     "id": "vermelho_escuro",
     "rotulo": "29y · ruiva escura + argola na narina",
    ...
    {"idade": 26, "corpo": "tall and statuesque with a long waist",
     "id": "morango_tranca",
     "rotulo": "26y · tranca loira rosada + mecha prata",
     "id": "morango_tranca",
     "rotulo": "26y · tranca loira rosada + mecha prata",
    ...
    {"idade": 25, "corpo": "slim and toned with a dancer's line",
     "id": "preto_risca_meio",
     "rotulo": "25y · preto risca ao meio + malar forte",
     "id": "preto_risca_meio",
     "rotulo": "25y · preto risca ao meio + malar forte",
    ...
    {"idade": 28, "corpo": "long-legged and slender with square shoulders",
     "id": "platinado_chanel",
     "rotulo": "28y · loira platinada + olhos gelo",
     "id": "platinado_chanel",
     "rotulo": "28y · loira platinada + olhos gelo",
    ...
    {"idade": 24, "corpo": "curvy and athletic with a small waist",
     "id": "cachos_altos",
     "rotulo": "24y · cachos presos no alto + sorriso",
     "id": "cachos_altos",
     "rotulo": "24y · cachos presos no alto + sorriso",
    ...
    {"idade": 30, "corpo": "tall and slim with an hourglass line",
     "id": "castanho_praia",
     "rotulo": "30y · castanho ondas praia + covinha",
     "id": "castanho_praia",
     "rotulo": "30y · castanho ondas praia + covinha",
    ...
    {"idade": 27, "corpo": "lean and toned with a flat stomach and long arms",
     "id": "preto_cintura",
     "rotulo": "27y · preto liso ate a cintura + tarraxa",
     "id": "preto_cintura",
     "rotulo": "27y · preto liso ate a cintura + tarraxa",
    ...
    {"idade": 26, "corpo": "shapely with toned arms and a narrow waist",
     ...
     "rotulo": "26y · balayage caramelo + pinta no olho",
     "id": "balayage_caramelo",
     "rotulo": "26y · balayage caramelo + pinta no olho",
    ...
    {"idade": 23, "corpo": "slim-hipped and elegant with a long neck",
     "id": "areia_espinha",
     "rotulo": "23y · loira areia + tranca espinha",
     "id": "areia_espinha",
     "rotulo": "23y · loira areia + tranca espinha",
    ...
    {"idade": 31, "corpo": "curvy and strong with a small waist",
     "id": "box_braids",
     "rotulo": "31y · box braids + argola dourada",
     "id": "box_braids",
     "rotulo": "31y · box braids + argola dourada",
    ...
    {"idade": 25, "corpo": "tall and lean with swimmer's shoulders",
     ...
     "rotulo": "25y · rabo auburn baixo + olhos cinza",
     "id": "auburn_rabo_baixo",
     "rotulo": "25y · rabo auburn baixo + olhos cinza",
    ...
    {"idade": 28, "corpo": "softly curvy with a full figure and a narrow waist",
     "id": "castanho_lateral",
     "rotulo": "28y · castanho risca lateral + sinal",
     "id": "castanho_lateral",
     "rotulo": "28y · castanho risca lateral + sinal",
    ...
    {"idade": 24, "corpo": "slim and supple with a very straight back",
     "id": "cobre_bob",
     "rotulo": "24y · chanel cobre + olhos verdes",
     "id": "cobre_bob",
     "rotulo": "24y · chanel cobre + olhos verdes",
    ...
    {"idade": 29, "corpo": "long-limbed and shapely with a defined waist",
     "id": "preto_rabo_alto",
     "rotulo": "29y · rabo preto alto + sobrancelha cheia",
     "id": "preto_rabo_alto",
     "rotulo": "29y · rabo preto alto + sobrancelha cheia",
    ...
    {"idade": 26, "corpo": "trim and athletic with a flat stomach",
     "id": "loiro_dourado",
     "rotulo": "26y · loira dourada + uma covinha so",
     "id": "loiro_dourado",
     "rotulo": "26y · loira dourada + uma covinha so",
    ...
    {"idade": 30, "corpo": "curvy with a small waist and long legs",
     "id": "coils_rente",
     "rotulo": "30y · coils rente + malar esculpido",
     "id": "coils_rente",
     "rotulo": "30y · coils rente + malar esculpido",
    ...
    {"idade": 27, "corpo": "slim with a long waist and square shoulders",
     "id": "cinza_coque",
     "rotulo": "27y · coque castanho cinza + pinta",
     "id": "cinza_coque",
     "rotulo": "27y · coque castanho cinza + pinta",
    ...
    {"idade": 25, "corpo": "shapely and toned with a narrow waist",
     "id": "ginger_cachos",
     "rotulo": "25y · cachos ginger + covinha no queixo",
     "id": "ginger_cachos",
     "rotulo": "25y · cachos ginger + covinha no queixo",
    ...
    {"idade": 28, "corpo": "tall and slim with a graceful neck",
     "id": "tranca_coroa",
     "rotulo": "28y · tranca coroa escura + pinta alta",
     "id": "tranca_coroa",
     "rotulo": "28y · tranca coroa escura + pinta alta",
    ...
    {"idade": 23, "corpo": "petite and shapely with a defined waist",
     "id": "loiro_oxigenado",
     "rotulo": "23y · chanel loiro oxigenado + queixo",
     "id": "loiro_oxigenado",
     "rotulo": "23y · chanel loiro oxigenado + queixo",
    ...
    {"idade": 31, "corpo": "athletic and curvy with strong shoulders",
     "id": "mogno_ondas",
     "rotulo": "31y · ondas mogno + argola no nariz",
     "id": "mogno_ondas",
     "rotulo": "31y · ondas mogno + argola no nariz",
    ...
    {"idade": 24, "corpo": "long-legged and lean with a flat stomach",
     ...
     "rotulo": "24y · rabo penteado para tras + sinal",
     "id": "castanho_penteado",
     "rotulo": "24y · rabo penteado para tras + sinal",
    ...
    {"idade": 26, "corpo": "curvy and confident with a very narrow waist",
     "id": "ruivo_mel",
     "rotulo": "26y · ruivo mel + sardas densas",
     "id": "ruivo_mel",
     "rotulo": "26y · ruivo mel + sardas densas",
    ...
    {"idade": 29, "corpo": "slim and elegant with long arms",
     "id": "preto_ombro",
     "rotulo": "29y · preto no ombro + mecha branca",
     "id": "preto_ombro",
     "rotulo": "29y · preto no ombro + mecha branca",
    ...
    {"idade": 25, "corpo": "tall with a small waist and full shoulders",
     "id": "morango_coque",
     "rotulo": "25y · coque loiro rosado + olhos verdes",
     "id": "morango_coque",
     "rotulo": "25y · coque loiro rosado + olhos verdes",
    ...
    {"idade": 26, "corpo": "tall and athletic with a long waist and narrow hips",
     "id": "canela_tranca",
     "rotulo": "26y · tranca canela + olhos ambar",
     "id": "canela_tranca",
     "rotulo": "26y · tranca canela + olhos ambar",
    ...
    {"idade": 24, "corpo": "shapely and lithe with a very small waist",
     "id": "espresso_no",
     "rotulo": "24y · coque torcido espresso + cilios",
     "id": "espresso_no",
     "rotulo": "24y · coque torcido espresso + cilios",
    ...
    {"idade": 28, "corpo": "long-legged and toned with square shoulders and a flat stomach",
     "id": "trigo_lateral",
     "rotulo": "28y · loira trigo + argola na orelha",
     "id": "trigo_lateral",
     "rotulo": "28y · loira trigo + argola na orelha",
    ...
    {"idade": 23, "corpo": "petite and curvy with full shoulders and a defined waist",
     "id": "cornrows_coque",
     "rotulo": "23y · cornrows finas + coque baixo",
     "id": "cornrows_coque",
     "rotulo": "23y · cornrows finas + coque baixo",
    ...
    {"idade": 30, "corpo": "statuesque and strong with a narrow waist and long arms",
     "id": "cobre_queimado",
     "rotulo": "30y · cobre queimado longo + sardas",
     "id": "cobre_queimado",
     "rotulo": "30y · cobre queimado longo + sardas",
    ...
    {"idade": 27, "corpo": "slim and supple with a very long neck",
     ...
     "rotulo": "27y · preto azulado franja + mecha prata",
     "id": "azul_preto_franja",
     "rotulo": "27y · preto azulado franja + mecha prata",
...
# ---------------------------------------------------------------------------
# ⭐⭐ A CENA 2 DO 16 — QUATRO EIXOS, e o quarto e' o que so' o DUPLA tem
# ---------------------------------------------------------------------------
# ⛔⛔ NAO E' UMA LISTA DE FRASES PRONTAS, e a razao esta' escrita com sangue no
# §35 das licoes-de-construcao: no TRIO 16 eu entreguei uma lista unica de 16
# frases e ela tinha os dois defeitos que o operador achou lendo a saida —
# pronome sem dono (`she feels it first`) e a MESMA FORMA em 62% das entradas.
# Nenhum linter acusa forma repetida; e' mode-collapse com relatorio verde.
#
# ⭐ Aqui sao quatro eixos compostos:
#     PROVA (com o literal) · PROMESSA (parceira NOMEADA) · ISCA · FOLLOW
# ⚠️ MEDIDO antes de escrever: 25.200 combinacoes, 67% cabem em 25 palavras,
# ZERO entrada inalcancavel. As versoes de 5 e de 3 eixos foram medidas junto e
# perderam — a de 5 ficava com mediana colada no teto (25), a de 3 jogava fora
# a parceira, que e' diretiva do operador.

# ⭐ A PROVA. Toda entrada carrega o literal `gelatin trick` como SUJEITO.
# ⛔ E ISSO E' OBRIGATORIO NESTE MOTOR, nao estilo: medido em 600 sorteios do
# DUPLA SHORT, o literal aparece na cena 1 em 234/600 e na cena 2 em 600/600
# (via ANCORAS). Com a cena 2 reescrita, se a PROVA nao o carregasse, 61% dos
# videos sairiam sem o termo que amarra o criativo a' VSL.
# ⭐ Como sujeito ele ja' esta' no topo da hierarquia (§31) sem precisar do
# rotulo com dois pontos — a lente `lint_hierarquia_mecanismo` so' dispara
# quando ha' enumeracao antes dele, e aqui nao ha'.
PROVAS = [
    ...
    "The gelatin trick makes your {o} thick",
    "The gelatin trick loads your {o} again",
    "The gelatin trick keeps your {o} up",
    "The gelatin trick puts your {o} back",
    "The gelatin trick hardens your {o} again",
]

# ⭐⭐ A PROMESSA — e a parceira e' SEMPRE NOMEADA.
# ⛔ Diretiva do operador, 2026-08-08: *"concretude, ser taxativo, e' melhor
# candidato que pronomes genericos (pool de 'sua namorada', 'sua esposa')"*.
# `she` obriga o espectador a descobrir de quem se trata no meio de 8 segundos;
# `your wife` entrega no primeiro fonema — e ainda SEGMENTA, porque casado e
# namorando nao ouvem a mesma promessa.
# ⛔ TODA entrada e' COMPORTAMENTO OBSERVAVEL, nunca estado interno. `she is
# satisfied` nao se ve'; `stops faking it` e' um plano. O espectador compra a
# cena, nao o adjetivo.
# ⚠️ O `PROMESSAS` do DUPLA SHORT, que morreu aqui, tinha entradas com `she`
# nu (`She will feel your {o} before she says anything`) — o mesmo defeito, na
# producao dele. Fica reportado, nao corrigido: copy e' alcada do operador.
PROMESSAS16 = [
    "Your wife stops faking it",
    "Your girl tells her friends",
    ...
    "Your woman stops pretending",
]

# ⭐⭐ O COMANDO. O NUCLEO e' fixo e a ISCA e' sorteada — e' o que separa este
# motor do TRIO 16, onde o payload do CTA e' fixo. `ISCAS_ENTREGA` sobreviveu
# inteiro, com as 18 entradas.
# ⛔⛔ A VIRGULA DEPOIS DE `gelatin` E' O ITEM MAIS IMPORTANTE DESTE ARQUIVO. O
# operador confirmou que a automacao de DM casa palavra EXATA; se a boca disser
# `comment gelatin and follow`, o espectador digita isso e nao dispara nada. A
# legenda nasce do Whisper EM CIMA DO AUDIO — nao ha' conserto depois. A
# virgula fecha o token e o follow vem em FRASE PROPRIA. Lente T16-2.
CTA_NUCLEO = "%s and I'll send you" % sc.CTA_LITERAL

# ⭐ O FOLLOW — curto por ARITMETICA, nao por estilo. Ver o controle [ALCANCE]
# no autoteste: entrada que nao cabe somada aos minimos dos outros eixos nunca
# e' sorteada, e o autoteste a contaria como opcao viva. No TRIO 16 seis de dez
# entradas estavam nessa condicao e so' a medicao de cobertura pegou.
FOLLOWS = [
    "Followers only.",
    "Follow first.",
    "Follow me first.",
...
# ⭐ DUAS CENAS. O teto continua 25 — ele nao vem do numero de cenas, vem
# da fisica: 8 segundos x 3,1 palavras/s. Encurtar o video nao alarga a
# boca. O `durationSeconds: 10` do AdBatch e' aspiracional; medido em
# campo, o player do Flow marca 00:08:00.
# ⛔ O piso da cena 2 e' ARITMETICA: menor PROVA + menor PROMESSA + nucleo
# do CTA + menor ISCA + menor FOLLOW. Piso abaixo disso e' alarme que
# nunca toca, que e' pior que alarme nenhum.
TETO_FALA = {1: 25, 2: 25}
PISO_FALA = {1: 18, 2: 20}
...
# texto -> `id`, entao o operador escolhe "31y · box braids + argola dourada" e
# o motor recebe `box_braids`. Um menu de `cabeca` seria um paragrafo em ingles
# por linha — ilegivel num combobox de 38 colunas.
# ⚠️ E aqui a IDADE no rotulo carrega informacao de verdade, ao contrario do
# TRIO: este pool vai de 22 a 31 anos, entao o prefixo separa duas mulheres que
# o cabelo sozinho nao separaria.
...
def _falas(spec, rng, quais=(0, 1)):
    ...
    ⭐ A COTA DO ORGAO E' GARANTIDA NO SORTEIO, nao so' cobrada no linter: a
    cena 1 o nomeia pelo PROBLEMA (`If your {o} looks like this one`) e a cena
    2 pela PROVA (`The gelatin trick gets your {o} hard`). Linter que reprova
    ...
    if 1 in quais:
        # ⭐⭐ A CENA 2 DO 16 — QUATRO BEATS EM 25 PALAVRAS:
        #   PROVA     o `gelatin trick` como sujeito + o orgao
        #   PROMESSA  a parceira NOMEADA + um comportamento observavel
        #   CTA       `Comment gelatin, and I'll send you <isca>`
        #   FOLLOW    em frase PROPRIA, nunca colado na keyword
        #
        # ⛔⛔ A ORDEM PROTEGE A KEYWORD. O `follow` vem DEPOIS do ponto final
        # do CTA. Automacao de DM casa palavra EXATA (operador, 2026-08-08): se
        # a boca disser `comment gelatin and follow`, o espectador digita isso
        # e o DM nunca sai. A legenda nasce do Whisper em cima do audio — nao
        # ha' conserto depois de gerado.
        #
        # ⛔ PONTO FINAL entre a prova e a promessa, nao `, and`. Medido no TRIO
        # 16: com o conector cabiam 69% das combinacoes, com o ponto 87% — e
        # duas afirmacoes curtas batem mais forte que uma composta, que e' o
        # que o operador cobra.
        #
        # ⚠️ `o2` na prova: sao dois orgaos diferentes no mesmo video, e repetir
        # o substantivo em 16 segundos vira bordao mais rapido que em 24.
        def _c2(pr, pm, isca, fol):
            return "%s. %s. %s %s. %s" % (pr.format(o=o2).rstrip(". "),
                                          pm.rstrip(". "), CTA_NUCLEO,
                                          isca, fol)

        c_pm = min(PROMESSAS16, key=_palavras)
        c_is = min(ISCAS_ENTREGA, key=_palavras)
        c_fo = min(FOLLOWS, key=_palavras)
        pr = rng.choice(_cabem(PROVAS,
                               lambda x: _c2(x, c_pm, c_is, c_fo), TETO_FALA[2]))
        pm = rng.choice(_cabem(PROMESSAS16,
                               lambda x: _c2(pr, x, c_is, c_fo), TETO_FALA[2]))
        isca = rng.choice(_cabem(ISCAS_ENTREGA,
                                 lambda x: _c2(pr, pm, x, c_fo), TETO_FALA[2]))
        fol = rng.choice(_cabem(FOLLOWS,
                                lambda x: _c2(pr, pm, isca, x), TETO_FALA[2]))
        f[1] = _c2(pr, pm, isca, fol)
    return f
...
# Achado LENDO o prompt gerado, 2026-08-08. Cinco das seis entradas de PARES
# trazem `held upright` dentro do campo `gigante`, e as strings que posicionam o
# objeto acrescentam a sua. No DUPLA o resultado e'
#
#     "an enormous geoduck clam, ..., held upright raised at chest height"
#
# em 300 de 300 blocos — e o `dupla_short.py` tem a MESMA taxa, entao e' defeito
# HERDADO, nao introduzido aqui.
# ⚠️ A primeira sonda que escrevi procurava a string exata `held upright, held
# upright` e deu 0/300: no DUPLA a segunda ordem e' `raised at chest height`,
# outra string, mesmo defeito. Lente mais estreita que o alvo mede a coisa
# errada e devolve verde.
# ⚠️ Por que importa e nao e' so' feio: `held upright` e `raised at chest
# height` nao sao a mesma coisa — a segunda tem ALTURA, a primeira nao. Duas
# ordens de pose para o mesmo objeto e' ordem ambigua, e o Veo escolhe uma.
...
def montar(spec):
    ...
    # --- CENA 2 — A BANCADA, O PROP GIGANTE, O COPO E A AMIGA ---------------
    # ⭐⭐ E' A FUSAO DAS CENAS 2 E 3 DO DUPLA, e ela nao inventa cenario: as
    # TRES cenas deste angulo ja' acontecem na mesma cozinha, e a cena 3 abria
    # com `same place, same background`. O que muda e' que agora cabem juntas:
    #   · a BANCADA da receita, com o aparato da conformacao sorteada, o raro e
    #     a tigela de cubos de gelatina — a prova de que a receita foi feita, e
    #     o lugar onde o mecanismo mora (DU2);
    #   · ela com o PROP GIGANTE numa mao e o COPO na outra, que era a cena 3
    #     inteira e e' ordem literal do operador (*"uma delas segurando o
    #     geoduck ereto e bem grande"*);
    #   · a AMIGA atras, muda, com a reacao sorteada.
    #
    # ⛔ E AQUI A AMIGA FICA — ao contrario do TRIO 16, onde ela teve de sair
    # para o corpo-prova masculino caber. Neste angulo NAO HA' HOMEM: sao duas
    # pessoas e a bancada, menos gente do que o TRIO fundido tem. Tirar a amiga
    # daqui mataria o angulo, que se chama DUPLA.
    #
    # ⛔⛔ `aparato16`, NAO `bancada`. A string do DUPLA comeca com `a tall
    # clear glass` — o copo sendo enchido — e aqui o copo esta' NA MAO dela.
    # Dois copos altos no mesmo quadro e o Veo escolhe um, normalmente o da
    # bancada, e o objeto da keyword sai da mao. Lente T16-1.
    # ⛔ E o `acao` da conformacao NAO entra: ele descreve as maos dela
    # trabalhando o utensilio, e as duas maos dela estao ocupadas — uma com o
    # prop, outra com o copo. Take mandando animar gestos incompativeis e' a
    # contradicao IMAGE x TAKE que a `lint_take_vs_image` existe para pegar.
    _pr = spec["preparo"]
    b["IMAGE 02/02"] = (
        "Closer medium shot in the same place, same background, same %(luz_c)s, "
        "filmed straight on and framed from the waist up, with %(sup_a)s running "
        "across the bottom third of the picture. %(Ancora)s, standing centred "
        "behind it and turned towards the lens, holding %(gigante)s. Her other "
        "hand holds %(copo)s. On the %(sup)s in front of her stands %(bancada)s. "
        "She looks directly into the camera, calm and certain, her mouth open "
        "mid-word as she speaks, her front teeth even and complete. %(amiga)s "
        "%(anti)s %(cauda)s"
    ...
    mov = [
        ...
        # ⭐⭐ O TAKE 2 ANIMA UM QUADRO PARADO, e e' isso mesmo: as duas maos
        # dela estao ocupadas (prop numa, copo na outra) e a bancada e' prova,
        # nao acao.
        # ⛔ O `mov` da conformacao NAO entra. Ele descreve as maos dela
        # trabalhando o utensilio — e nao ha' mao livre. Mandar animar os dois
        # gestos e' a contradicao IMAGE x TAKE que ja' custou o lote da etapa
        # [7] deste angulo.
    ...
    # ⚠️ `frame-left speaks` E' CORRETO NESTE ANGULO, e foi MEDIDO: a
    # `BO_DUPLA` poe a REF em frame-left e ela e' quem fala. No TRIO a mesma
    # string estava ERRADA em 200 de 200, porque la' quem fala esta' EM PE
    # ATRAS e frame-left e' uma das sentadas. Herdar sem medir teria trocado a
    # boca aqui tambem — ou deixado o defeito de la' passar por certo.
    elenco = [
        ...
        # `BO_AMIGA_TAKE` termina em "Only the woman on frame-left speaks"
        # e serve a cena 1, onde as duas estao lado a lado. Na cena do
        # payoff a IMAGE diz "standing centred in the frame" — e o TAKE
        # mandava falar a de frame-left, que ali e' a AMIGA, muda por
        # construcao. MEDIDO: 300 de 300 sorteios, e o `dupla_short.py`
        # tem a mesma taxa. Contradicao IMAGE x TAKE e' pior que omissao:
        # a omissao o gerador preenche com o frame, a contradicao ele
        # resolve mexendo no que estava certo — aqui, na boca que fala.
...
def lint(spec, blocos):
    ...
    # A automacao de DM casa a palavra EXATA (operador, 2026-08-08), e a legenda
    # do video nasce do Whisper EM CIMA DO AUDIO: nao ha' conserto depois de
    # gerado. A regra e' POSICIONAL — depois de `comment gelatin` tem de vir
    # VIRGULA, seja qual for a palavra seguinte. Listar palavras proibidas seria
    # corrida perdida; a proxima seria `now`, `below`, `please`.
    # ⚠️ E e' `finditer`, nao `search`: `gelatin` aparece DUAS vezes na fala (no
    # `gelatin trick` e no comando), e olhar so' a primeira ocorrencia dava
    # verde para `Comment gelatin now,`. Foi assim no TRIO 16, e so' o controle
    # do autoteste pegou.
    _f2 = falas[1] or ""
    ...
    # namorada', 'sua esposa'"*.
    # ⚠️ O `PROMESSAS` do DUPLA SHORT tem entradas com `she` nu (`She will feel
    # your {o} before she says anything`). Aqui elas nao entram, e a lente
    # garante que nao voltem por copia.
    if not any(_p in _f2.lower() for _p in ("wife", "girlfriend", "woman", "girl")):
        ach.append(("ERRO", "T16-5: a cena 2 nao NOMEIA a parceira — sem "
                            "`wife`/`girlfriend`/`woman`/`girl` o espectador "
                            "gasta o segundo dele descobrindo de quem ela fala "
                            "(%r)" % _f2[:60]))
    ...
    if _vago:
        ach.append(("ERRO", "T16-5: %r — `it` sem antecedente depois de verbo de "
                            ...
                            "e' a diferenca entre vender e o espectador "
    ...
    # ⚠️ NO 16 a cena 2 abre com o `gelatin trick` como SUJEITO e nomeia o orgao
    # na mesma sentenca — os dois referentes, nao um. A lista aceita os dois
    # porque a copy pode legitimamente abrir por qualquer um deles.
    # ⛔ A entrada `(3, ...)` foi APAGADA, nao deixada para o `continue` engolir:
    # `falas` tem DOIS itens e `falas[2]` estourava IndexError. Lente que olha
    # cena inexistente e' sonda cega no melhor caso e crash no pior — aqui foi o
    # pior, e foi o autoteste que pegou na primeira rodada.
    ...
    # ⛔⛔ A COBRANCA DO RARO **FALADO** FOI APAGADA — 2026-08-08, POR MEDICAO.
    # No DUPLA SHORT o raro e' nomeado na fala da cena 2, pelas ANCORAS. Aqui o
    # beat da receita morreu na fusao, e eu medi se ele cabia antes de decidir:
    #
    #   com o raro dentro da PROVA   226.800 combos, so' 15% cabem em 25
    #                                palavras, mediana 25 (colada no teto) e
    #                                SEIS entradas de outros pools ficam
    #                                INALCANCAVEIS — 4 ISCAS_ENTREGA e 2 FOLLOWS
    #   sem ele                      25.200 combos, 67% cabem, mediana 24,
    #                                ZERO entrada morta
    #
    # Trocar 18 iscas e 10 follows por 9 raros e' perda liquida, e pool com
    # entrada inalcancavel e' mode-collapse com relatorio verde (o controle
    # [ALCANCE] existe por causa disso).
    # ⭐ O RARO NAO MORREU: ele continua no QUADRO, no `raro_img` da
    # `BANCADA16_CAUDA`, e continua sorteado, no ledger e no painel — o
    # `lint_painel_honesto` prova a presenca dele em cada bloco.
    # ⚠️ E' PERDA DE ENTROPIA FALADA, e esta' reportada como tal. Nao inventei
    # que "nao fazia falta": ela nao cabe.
    ...
    # resultado.
    # ⛔⛔ REAPONTADA, NAO APAGADA — 2026-08-08. Ela vigiava o pool `PROMESSAS`,
    # que morreu na fusao, e duas das tres cobrancas dela nao fazem mais sentido
    # onde estavam:
    #   · o `{o}` saiu da promessa e foi para a PROVA, que abre a cena 2. Cobrar
    #     o orgao dentro da promessa reprovaria 100% do pool novo.
    #   · a ULTIMA sentenca da cena 2 agora e' o FOLLOW, nao a promessa. Cobrar
    #     o orgao na ultima sentenca reprovaria 100% dos sorteios.
    # ⭐ O que a lente vigia continua sendo o CRITERIO DO OPERADOR, so' que no
    # slot certo: *"a linha vence quando empilha PROMESSA + DESEJO OCULTO, e o
    # desejo oculto mora na REACAO DELA"*. A involuntariedade e' a alavanca de
    # credibilidade — `my {o} is harder` e' alegacao, `your wife stops faking
    # it` e' evidencia.
    # ⛔ E ela agora cobra a PARCEIRA NOMEADA, nunca `she` nu: e' a diretiva de
    # 2026-08-08 aplicada no nivel do POOL, e nao so' na fala montada (T16-5).
    for linha in PROMESSAS16:
        if not re.search(r"\b(wife|girlfriend|woman|girl)\b", linha, re.I):
            ach.append(("ERRO", "BO15: promessa sem a PARCEIRA NOMEADA — %r. "
                                "Pronome generico obriga o espectador a "
                                "descobrir de quem se trata no meio de 8 "
                                "segundos, e ele desiste antes" % linha))
        if re.match(r"\s*(She|Her)\b", linha):
            ach.append(("ERRO", "BO15: promessa abrindo com pronome nu — %r"
                        % linha))
    # ⭐ O ORGAO E' COBRADO NA CENA INTEIRA, que e' onde ele de fato precisa
    # estar: a PROVA abre nomeando-o e a promessa entrega a reacao dela.
    ...
    # isto ela acusava 600 de 600 videos CERTOS. Lente que compara com a forma
    # do POOL em vez da forma do PROMPT inventa defeito, e foi exatamente o que
    # aconteceu no TRIO 16 na mesma hora do dia.
    ...
    # ⛔ A fusao junta a BANCADA e o COPO no mesmo frame. A string `bancada`
    # comeca com `a tall clear glass` — o copo sendo enchido — e aqui o copo
    # esta' NA MAO dela. Dois copos altos e o Veo escolhe um, normalmente o da
    # bancada, e o objeto da keyword sai da mao. Por isso existe o `aparato16`,
    # e por isso esta lente existe: um `aparato16` novo escrito com o copo
    # dentro passaria despercebido ate' o render.
    ...
    if "never speaks" not in blocos["TAKE 02/02"]:
        ach.append(("ERRO", "BO6: TAKE 02/02 sem a trava de mudez"))
    ...
    if "only person" in i2:
        ach.append(("ERRO", "BO6: a cena 2 declara pessoa UNICA e tem DUAS — "
    ...
    if "gelatin cubes" in i1:
        ach.append(("ERRO", "DU2: a tigela de gelatina na cena 1 — entrega "
                            "o mecanismo antes da promessa"))

    # --- T16-3: a POSE do prop nao pode vir duas vezes ----------------------
    # ⛔ Duas ordens de pose para o mesmo objeto e' ordem ambigua. Aqui a
    # segunda vem de `raised at chest height` (cena 1) ou do proprio campo
    # `gigante` (`held upright`), e as duas juntas apareciam em 300/300.
    _POSES = ("held upright", "raised at chest height", "held in her lap")
    for _n, _t in blocos.items():
        if not _n.startswith("IMAGE"):
            continue
        for _sent in re.split(r"(?<=[.;])\s+", _t):
            if sum(1 for _p in _POSES if _p in _sent) >= 2:
                ach.append(("ERRO", "T16-3: duas ordens de POSE no mesmo "
                                    "sintagma em %s — a pose mora na frase que "
                                    "POSICIONA, nunca dentro do prop; use "
                                    "`_sem_pose()`" % _n))
                break

    # --- T16-4: ⭐⭐ QUEM FALA NO TAKE E' QUEM FALA NA IMAGE ------------------
    # ⛔⛔ MEDIDO em 300 de 300 sorteios do DUPLA SHORT: a IMAGE do payoff poe a
    # REF `standing centred in the frame` e o TAKE mandava falar a de
    # `frame-left`, que ali e' a AMIGA — muda por construcao. Passou por todo o
    # linter do DUPLA sem uma acusacao, porque nenhuma lente comparava o elenco
    # do TAKE com a geometria da IMAGE.
    # ⚠️ Na cena 1 o `frame-left` e' CORRETO (as duas estao lado a lado e a REF
    # e' a da esquerda) — por isso a lente olha a cena do payoff, nao as duas.
    if "centred" in i2 and "frame-left speaks" in blocos["TAKE 02/02"]:
        ach.append(("ERRO", "T16-4: a IMAGE 02/02 poe a REF CENTRADA e o TAKE "
                            "02/02 manda falar a de `frame-left`, que ali e' a "
                            "amiga — o Veo resolve a contradicao trocando a "
                            "boca que fala"))
...
def autoteste(n=600):
    ...
    for nome, pool in (("MUNDOS", MUNDOS), ("METODOS", METODOS),
                       ...
                       ("PROVAS", PROVAS), ("PROMESSAS16", PROMESSAS16),
    ...
    for _n, _p in (("PROBLEMAS", PROBLEMAS), ("VIRADAS", VIRADAS),
                   ...
                   ("PROMESSAS16", PROMESSAS16),
                   ("ISCAS_ENTREGA", ISCAS_ENTREGA),
                   ("FOLLOWS", FOLLOWS), ("REFS", REFS), ("HOMENS", HOMENS),
    ...
    # ⛔ Criado no TRIO 16 depois de medir cobertura e achar SEIS de dez FOLLOWS
    # que nunca saiam: custavam 6-7 palavras e so' havia 5 de orcamento. O
    # autoteste contava dez opcoes e a producao tinha quatro — pool que mente
    ...
    # morta. Nenhuma outra lente pega isso: todas verificam a frase MONTADA,
    # que sempre cabe por causa do fallback do `_cabem`.
    _mP = min(_palavras(x.format(o="soldier")) for x in PROVAS)
    _mM = min(_palavras(x) for x in PROMESSAS16)
    _mI = min(_palavras(x) for x in ISCAS_ENTREGA)
    _mF = min(_palavras(x) for x in FOLLOWS)
    _fx = _palavras(CTA_NUCLEO)
    for _nome, _pool, _outros in (
            ("PROVAS", [x.format(o="soldier") for x in PROVAS], _mM + _mI + _mF),
            ("PROMESSAS16", PROMESSAS16, _mP + _mI + _mF),
            ("ISCAS_ENTREGA", ISCAS_ENTREGA, _mP + _mM + _mF),
            ("FOLLOWS", FOLLOWS, _mP + _mM + _mI)):
        _teto_eixo = TETO_FALA[2] - _fx - _outros
        _mortas = [x for x in _pool if _palavras(x) > _teto_eixo]
        if _mortas:
            ctrl.append("[ALCANCE] %d entrada(s) de %s nunca podem ser sorteadas "
    ...
    b163["IMAGE 01/02"] = b163["IMAGE 01/02"].replace(
        "raised at chest height", "held upright raised at chest height", 1)
    if not any("T16-3" in msg for _, msg in lint(s, b163)):
        ctrl.append("[T16-3] NAO acusa a pose duplicada do prop")

    # ⭐⭐ [T16-4] a boca errada no payoff — 300/300 no dupla_short.py
    b164 = dict(b)
    b164["TAKE 02/02"] = b164["TAKE 02/02"].replace(
        "Only the woman standing centred in the frame speaks",
        "Only the woman on frame-left speaks")
    if not any("T16-4" in msg for _, msg in lint(s, b164)):
        ctrl.append("[T16-4] NAO acusa o TAKE mandando falar a mulher errada no "
                    "payoff — e' o defeito de 300/300 do DUPLA SHORT")

    # ⭐⭐ [T16-2] A KEYWORD GRUDADA — a sonda mais importante deste motor.
    s162 = dict(s, falas=list(s["falas"]))
    s162["falas"][1] = ("The gelatin trick gets your tool hard. Your wife stops "
                        "faking it. Comment gelatin and follow, and I'll send "
                        "you the recipe. Followers only.")
    if not any("T16-2" in msg for _, msg in lint(s162, b)):
        ctrl.append("[T16-2] NAO acusa `comment gelatin and follow` — a keyword "
                    "grudada volta a passar e o DM nao dispara")
    s162b = dict(s, falas=list(s["falas"]))
    s162b["falas"][1] = s162b["falas"][1].replace("Comment gelatin,",
                                                  "Comment gelatin now,")
    if not any("T16-2" in msg for _, msg in lint(s162b, b)):
    ...
    s165["falas"][1] = ("The gelatin trick puts your tool back. She feels it "
                        "first. " + CTA_NUCLEO + " the recipe. Followers only.")
    ...
    # ⛔ O CONTROLE DE BO8 SAIU JUNTO COM A COBRANCA DO RARO FALADO. Ele
    # sabotava `falas[1]` trocando o nome do raro por "something" e esperava
    # acusacao — com a regra apagada ele nunca podia disparar. Sonda que nao
    # pode falhar e' decoracao, e decoracao faz o autoteste inteiro perder o
    # direito de ser acreditado: ele imprime "OK" do mesmo jeito.
    # ⭐ O que substitui a protecao: o `lint_painel_honesto` continua provando
    # que o `raro` sorteado chega ao QUADRO, que e' onde ele vive agora.
    ...
    b6b = dict(b)
    # ⛔⛔ A ANCORA E' O `CAUDA`, NAO O `ANTICELEB` — trocada em 2026-08-14.
    # O plantio usava o ANTICELEB como ponto de insercao, e o ANTICELEB era
    # justamente a clausula que a remocao anti-celebridade iria esvaziar em
    # outros motores. ⚠️ `str.replace("", X)` INSERE X entre cada caractere:
    # simulado antes da troca, um bloco de 91 caracteres virava 5.059 — e o
    # controle continuava "passando", pelo motivo errado, sobre um texto que
    # virou lixo. Ancora de controle negativo tem de ser literal ESTAVEL.
    b6b["IMAGE 02/02"] = b6b["IMAGE 02/02"].replace(
        CAUDA, "Behind them stands a 60-year-old man, his mouth open. " + CAUDA)
    ...
    b6c = dict(b)
    b6c["IMAGE 02/02"] = b6c["IMAGE 02/02"].replace(
    ...
    # medida — e neste motor a funcao ESTAVA quebrada: o pool nao tinha `id` e o `sortear` resolvia a
    # trava por `cabeca`, entao o menu nasceria mudo.
...
def main():
    ap = argparse.ArgumentParser(
        description="Randomizador do agente DUPLA 16 (2 takes de 8s)")
