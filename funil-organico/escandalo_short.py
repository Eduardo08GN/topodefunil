...
LEDGER = os.path.join(AQUI, ".escandalo-short-ledger.json")

TITULO = "AGENTE ESCANDALO SHORT"
SUBTITULO = "o escandalo alheio — a cara de choque que reage no lugar do espectador · 3 cenas"
SLUG = "escandalo-short"

CENAS_UI = ["1 · O ESCANDALO", "2 · A RECEITA INCOMPLETA", "3 · A PROVA + CTA"]
...
NARRADORAS = [
    ...
    # como ancora permitida desde 2026-08-04, e ela e' da mesma familia do
    # dente lascado que fez o operador reprovar o lote do PLACA 16 (*"esses
    ...
    # -----------------------------------------------------------------------
    # + 2026-08-13: OITO narradoras novas (20 -> 28). Ordem do operador:
    # *"melhore a aparencia e shape desses homens"* e *"aumente o pool de
    # opcoes substancialmente, tambem dos ambientes"*. ⚠️ Vinte entradas com
    # um sorteio que so' evita as 3 ultimas devolvem a mesma cara a cada
    # quatro videos, e quem ve o lote inteiro de uma vez e' o operador.
    # ⛔ AS TRAVAS DO CABECALHO VALEM SEM EXCECAO nas oito: 28-35 anos, PORTE
    # em 100%, ZERO oculos (a excecao do `medir_personagens` existe por
    # ordem — oculos de leitura brigam frontalmente com a LEI DO REF), ZERO
    # grisalho, ZERO pele castigada, ancora facial sempre SINAL DE BELEZA.
    # ⛔ Zero `scar`, `gap between teeth`, `sun spots`, `weathered`, `sunken`:
    # e' a lista que ja' custou lote no PLACA 16 e nao e' negociavel.
    # ⛔ Zero cor de pele — a etnia vem da PAGINA e ja' entra na frase montada;
    # escrever cor aqui poe duas vozes no mesmo sintagma e o gerador inventa.
    # ⚠️ Os quatro eixos que o medidor le' giram nas oito: cabelo (mecha,
    # pixie, ondas, bantu knots, trancado, blunt cut, coils, liso), porte
    # (lean, compact, tall, firm, broad-shouldered, short powerful), pele
    # (smooth-skinned, freckles, lightly tanned) e ancora (streak, dimple,
...
CENARIOS = [
    ...
    # -----------------------------------------------------------------------
    # + 2026-08-13: NOVE ambientes novos (15 -> 24), e os dois irmaos voltam a
    # ter o MESMO conjunto de ids. Ordem do operador: *"aumente o pool de
    # opcoes substancialmente, tambem dos ambientes"*.
    # ⚠️⚠️ OS CINCO PRIMEIROS SAO OS DO `escandalo16_short` (o bloco ES19 de
    # 2026-08-10, que nasceu so' la'): os irmaos tinham divergido em 20 x 15
    # e ninguem sabia qual era a fonte da verdade. Igualados POR CIMA.
    # ⛔ Eles entram SEM a chave `prep`: la' o take 2 filtra o cenario por ela
    # (`[c for c in CENARIOS if c.get("prep")]`) e aqui esse filtro nao
    # existe. Chave que o pool nao usa e' chave que a proxima edicao acha que
    # e' regra — a entrada nova declara exatamente as chaves das vizinhas.
    # ⚠️ CADA UMA TRAZ O AMBIENTE INTEIRO, no mesmo nivel das vizinhas: `set`
    # com superficie + dois objetos de leitura + a BANDEIRA, `re_ancora` que
    # amarra as cenas 2 e 3 a' 1 pelo traco mais reconhecivel + a bandeira, e
    # `luz` propria.
    # ⛔ ES13: a bandeira sai por `a|an|the [small|large] US flag` — nada de
    # adjetivo no meio, senao a clausula NAO sai no sorteio sem bandeira.
    # ⛔ ES14: zero planta, zero carpete, zero caixa triangular de bandeira —
    # por isso `clay jugs` e nao `clay pots` no rancho: `pot` puxa `pot plant`.
    # -----------------------------------------------------------------------
    ...
    {"id": "bancada_acougue", "bancada": "butcher block", "curto": "kitchen",
     ...
     # adjetivo no meio a clausula NAO sai no sorteio "sem bandeira".
...
BANCADAS = [
    ...
    # -----------------------------------------------------------------------
    # + 2026-08-13: SETE recibos novos (15 -> 22). Ordem do operador:
    # *"aumente o pool de opcoes substancialmente"*.
    # ⚠️ O pool efetivo NAO e' o tamanho da lista: o `_bancada_livre` corta
    # tudo que colide com a fala, com as `cabecas` da receita e com a imagem
    # dela, e so' entao evita as recentes. Com 15 entradas e 16 receitas
    # sobravam poucas por sorteio e o mesmo recibo voltava — ampliar aqui e'
    # ampliar o que sobra DEPOIS do filtro, que e' o numero que importa.
    # ⛔ `cabecas` declara o que a imagem deixa NOMEAVEL, e existe para o
    # sorteio evitar a colisao POR CONSTRUCAO: recibo que repete o {r} da boca
    # mostra dois ingredientes, nao quatro.
    # ⛔ Zero marca legivel: FORMA no lugar de rotulo, e a ausencia declarada
    # pela AFIRMATIVA (`plain`), nunca por `with no label` — negacao injeta
    # `label` num prompt cuja tese e' que nao ha' rotulo nenhum.
    # -----------------------------------------------------------------------
...
def _montar_falas(rng, par, receita, orgaos, relacao, degrau):
    ...
    deixaria ar num take de 8s, e ar vira pausa morta. Por isso cena 1 = hook +
    fecho, cena 2 = fundida + selo, cena 3 = prova + CTA + gate.

    Quatro filtros por construcao, todos com varredura de fallback:
    · o DEGRAU do hook e' escolha do Ed (ES10), nao do sorteio
    · o PISO e o TETO por cena (ES16) — piso nao cobrado e' piso que nao existe
    · a voz da PROVA x a relacao nomeada da cena 3 (ES4)
    · eco de FATO dentro do video inteiro (ES20)
    """
    ...
    # ⭐ A cena 3 abre com PROVA (ordem do Ed), e a prova NOMEIA o orgao com
    # `{o}` — sem ele ela diz que algo mudou e nao diz o que.
    prova_pool = [p for p in PROVAS
                  if voz == "intima" or p["voz"] == "terceiro"] or PROVAS
    prova = _escolher(rng, prova_pool,
                      lambda p: not _eco(c1, c2, p["txt"].format(o=orgaos[2])))
    prova = prova["txt"].format(o=orgaos[2])
    cta = rng.choice(CTAS)
    # ⚠️ o gate fecha a cena E o orcamento: escolhe-se o que ainda cabe entre o
    # piso e o teto da cena 3 depois da prova e do CTA. Sem esta checagem o take
    # 3 estoura (ou encolhe) em silencio, e o linter acusaria com o lote pronto.
    # ⚠️ E o gate cobre a P22 quando a prova e o CTA saem os dois em 3a pessoa:
    # e' o unico dos tres slots da cena 3 com folga para isso.
    falta_p22 = not (_aterrissa(prova) or _aterrissa(cta))
    gate = _escolher(
        rng, GATES,
        lambda g: (not _eco(c1, c2, prova, cta, g)
                   and PISO_FALA[3]
                   <= _palavras("%s %s %s" % (prova, cta, g)) <= TETO_FALA[3]
                   and not (falta_p22 and not _aterrissa(g))))
    c3 = "%s %s %s" % (prova, cta, gate)
    return [c1, c2, c3]
...
def montar(spec):
    ...
    par, rec, fis = spec["par"], spec["receita"], spec["fisica"]
    mec, ban, rea = spec["mecanismo"], spec["bancada"], spec["reacao"]
    falas = spec["falas"]
    bnc = cen["bancada"]
    ...
    recibo = ES_BANCADA_RECIBO % (bnc, ban["itens"])
    ...
    b["IMAGE 02/03"] = (
        "IMAGE 02/03: Medium shot at the %s in %s, same light. %s, stands "
    ...
    # --- IMAGE 03/03 — A PROVA + CTA -----------------------------------------
    # ⭐ [D1]/ES4 — a F12b. Ele DE PE, ativo, prop grande, nas PROPRIAS maos; ela
    # aponta SEM ENCOSTAR. O que bloqueia nao e' o prop, e' a AGENCIA — e a
    # agencia se declara tambem pelo OLHAR DELE NA LENTE, nao so' pelos punhos.
    # ⭐ [D2]/ES3 — e' o MESMO homem da cena 1, com a descricao inteira: e' isso
    # que o transforma de figurante em ARCO.
    # ⚠️ A ancora de escala e' NO CORPO DELE (`e_img_dele`): quem segura e' ele.
    ...
    # deterministicas de 2026-07-30.
    # ⛔ SEM bancada-recibo (ES14): e' o bloco mais arriscado do lote. No lugar
    # dela entra o MECANISMO na mao livre dela (ES9) — a cena que diz "comment
    # gelatin" passa a ter gelatina em quadro, e no frame exato da keyword.
    # ⛔ ES14/F12c, correcao de 2026-08-02 — este e' o bloco MAIS DENSO do agente
    # (246 palavras medidas, contra 92-236 nos irmaos SHORT) e era o unico sem
    # guarda. Saiu daqui o que era REDUNDANCIA PURA, nunca regra:
    #   · a relacao nomeada, que era dita duas vezes ("she is his wife of oito
    #     anos" + a mesma frase dentro da travada da F12b duas oracoes depois);
    #   · a FRASE_SEM_MARCA, exigida por engano num bloco que NAO tem
    #     bancada-recibo e cujo unico recipiente ja' e' descrito por forma —
    #     linha declaratoria e redundante escrita para tranquilizar um
    #     classificador que nao le' tranquilizacao (§Declaracao e' municao).
    # ⛔ A travada da F12b nao encolheu uma virgula: descricao livre encolhe,
    # string validada e' intocavel.
    b["IMAGE 03/03"] = (
        "IMAGE 03/03: Medium shot in %s, same light. %s, stands frame-left. The "
    ...
    b["TAKE 02/03"] = (
        ...
        "at any point in this shot. %s She is the only person in the shot.\n"
        "Dialogue: \"%s\"\nAudio: quiet room tone in the %s, one soft clink of a "
        "spoon against glass. No music."
        % (ES_RECEITA_TAKE % (rec["gesto"], fis["desc"], mec["curto"]),
           sonorizar(falas[1]), cen["curto"])
    )

    # --- TAKE 03/03 -----------------------------------------------------------
    # ⛔ ES15: so' ela tem Dialogue. O dialogo do Veo e' monofonico na pratica e
    # duas vozes saem tortas.
    b["TAKE 03/03"] = (
...
ES14_TETO_BLOCO = {"IMAGE 01/03": 275, "IMAGE 02/03": 230, "IMAGE 03/03": 255}
...
def _es1_plateia(spec, blocos, achados):
    ...
    """
    img, take = blocos["IMAGE 01/03"], blocos["TAKE 01/03"]
    ...
    if spec["figurantes"] == 2:
        ...
        elif ES_CONTRASTE_FIGURANTES not in blocos["IMAGE 01/03"]:
...
def _es3_arco(spec, blocos, achados):
    ...
    if "The same %d-year-old %s man" % (hom["idade"], et) not in blocos["IMAGE 03/03"]:
...
def _es5_sem_dose(spec, blocos, achados):
    ...
    for nome in ("IMAGE 02/03", "TAKE 02/03", "IMAGE 03/03", "TAKE 03/03"):
        alvo = _direcao(blocos[nome])
...
def _es8_recibo(spec, blocos, achados):
    ...
    if M_RECIBO not in blocos["IMAGE 02/03"] or itens not in blocos["IMAGE 02/03"]:
    ...
    if FRASE_SEM_MARCA not in blocos["IMAGE 02/03"]:
...
def _es9_keyword(spec, blocos, achados):
    ...
    """
    img3, take3 = blocos["IMAGE 03/03"], blocos["TAKE 03/03"]
...
def _es14_densidade(spec, blocos, achados):
    ...
    for nome, teto in sorted(ES14_TETO_BLOCO.items()):
        ...
        if nome == "IMAGE 01/03" and spec["figurantes"] == 2:
...
def _es15_elenco(spec, blocos, achados):
    ...
    """
    if "She is the only person in the frame." not in blocos["IMAGE 02/03"]:
        achados.append(("ERRO", "ES15: IMAGE 02/03 nao declara elenco 1 — o corte "
                                "seco descarta o hook inteiro"))
    if "They are the only two people in the frame." not in blocos["IMAGE 03/03"]:
        achados.append(("ERRO", "ES15: IMAGE 03/03 nao declara elenco 2"))
    for nome in ("TAKE 01/03", "TAKE 03/03"):
        if "Only she speaks." not in blocos[nome]:
            achados.append(("ERRO", "ES15: %s sem 'Only she speaks.' — o dialogo "
                                    ...
                                    "tortas" % nome))
    for nome in ("IMAGE 02/03", "TAKE 02/03"):
        if spec["par"]["e_img"] in blocos[nome] or \
                _achar(_direcao(blocos[nome]), (spec["par"]["f_nome"],)):
            achados.append(("ERRO", "ES15: %s ainda carrega os props do hook — "
                                    "eles somem no corte e nao voltam" % nome))
...
def _es19_gates(spec, blocos, achados):
    ...
    """
    hits = _achar(spec["falas"][2], VOCATIVOS)
    if len(hits) > 1:
...
def lint(spec, blocos):
    ...
    # ⚠️ `cota_min=2`: nos degraus 1 e 4 a cena 1 nao carrega substantivo do
    # nucleo por construcao, entao as cenas 2 e 3 seguram a cota sozinhas — e as
    # duas o carregam em 100% do pool.
...
def resumo_pt(spec):
    ...
    return (
        ...
        "sozinha, prepara %s em água morna e declara a receita incompleta sem o "
        "gelatin trick. Na cena 3 o MESMO homem de %d anos segura o eixo nas "
...
def _orgaos_de(spec):
    ...
    return [sc.orgao_de(sys.modules[__name__], spec["falas"][0]),
            sc.orgao_de(sys.modules[__name__], spec["falas"][1],
                        padrao="soldier"),
            sc.orgao_de(sys.modules[__name__], spec["falas"][2],
                        padrao="pecker")]
...
def _trocar_homem(spec, rng):
    ...
    if voz_da_relacao(spec["relacao"]) != "intima":
        spec["falas"][2] = nova_fala(spec, 2, rng)
        ...
        for _ in range(n_por_pagina):
            ...
            for nivel, msg in lint(spec, blocos):
                if nivel == "ERRO":
                    ...
                    avisos += 1
            if m is not None:
                if "brother" in spec["falas"][2].lower():
                    m["vicio"] += 1
                for i, fala in enumerate(spec["falas"], 1):
                    if not _aterrissa(fala):
                        m["p22"][i] += 1
...
def autoteste(n_por_pagina=80, seed=7):
    ...
    extra_r = max(_palavras(r["fala"]) for r in RECEITAS) - 1
    for d in DEGRAUS:
        hk = [_palavras(h["txt"]) for h in HOOKS if h["degrau"] == d]
        fe = [_palavras(f) for f in FECHOS]
        ex = extra_o if d in (2, 3) else 0
        if min(hk) + max(fe) < PISO_FALA[1]:
            falhas.append("ES16: cena 1 no degrau %d nao alcanca o piso (%d < %d)"
                          % (d, min(hk) + max(fe), PISO_FALA[1]))
        if max(hk) + min(fe) + ex > TETO_FALA[1]:
            falhas.append("ES16: cena 1 no degrau %d pode estourar (%d > %d)"
                          % (d, max(hk) + min(fe) + ex, TETO_FALA[1]))
    fu = [_palavras(f) for f in FUNDIDAS]
    se = [_palavras(s) for s in SELOS]
    if min(fu) + max(se) < PISO_FALA[2]:
        falhas.append("ES16: cena 2 nao alcanca o piso (%d < %d)"
                      % (min(fu) + max(se), PISO_FALA[2]))
    if max(fu) + min(se) + extra_o + extra_r > TETO_FALA[2]:
        falhas.append("ES16: cena 2 pode estourar (%d > %d)"
                      % (max(fu) + min(se) + extra_o + extra_r, TETO_FALA[2]))
    pr = [_palavras(p["txt"]) for p in PROVAS]
    ct = [_palavras(c) for c in CTAS]
    ga = [_palavras(g) for g in GATES]
    if min(pr) + min(ct) + max(ga) < PISO_FALA[3]:
        falhas.append("ES16: cena 3 nao alcanca o piso (%d < %d)"
                      % (min(pr) + min(ct) + max(ga), PISO_FALA[3]))
    if max(pr) + extra_o + max(ct) + min(ga) > TETO_FALA[3]:
        falhas.append("ES16: cena 3 pode estourar (%d > %d)"
                      % (max(pr) + extra_o + max(ct) + min(ga), TETO_FALA[3]))

    # --- ES6/ES7, cobertura por ENUMERACAO do pool de fundidas -------------
    for i, f in enumerate(FUNDIDAS, 1):
        baixo = f.lower()
        if "gelatin trick" not in baixo:
            falhas.append("ES6: FUNDIDAS[%d] sem o literal 'gelatin trick'" % i)
        if "{o}" not in f or "{r}" not in f:
            falhas.append("ES6: FUNDIDAS[%d] sem slot {o} ou {r}" % i)
        n_tec = sum(baixo.count(t) for t in ES7_TECNICAS)
        if n_tec != 1:
            falhas.append("ES7: FUNDIDAS[%d] com %d palavras tecnicas" % (i, n_tec))
